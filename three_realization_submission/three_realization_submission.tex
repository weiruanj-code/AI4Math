...
Kummer torsors, with the induced tame transport on its actual log
realization and no additional wild
...
reductions.  The trace row is an equivalence in $D(\mathbf Z_\ell)$
and its corresponding star-algebra module category.  The actual
logarithmic star comparisons retain their induced tame transport;
additional supplied finite semilinear actions are treated in
Proposition~\ref{prop:frag-S-trace-naturality}, not by silently adding
an infinite inertia action to the finite-carrier target.  For input Tor-amplitude $[a,b]$, their perfect output
...
on the full selected constructible source.  Its star computation is
not restricted by the coefficient-duality domain: over a general
base, dual closure is asserted only for globally lisse-perfect inputs,
and Verdier partners require a coherent bounded dualizing system.
At the geometric point these qualifications do not shrink the
source or the supported conclusion of the main theorem.
...
\paragraph{Point outputs and the three action data.}
The point-valued adic target is $\varprojlim_r D(R_r)$; its uniformly
perfect cartesian systems recover in $D(\mathbf Z_\ell)$.
The star algebra is $A_\bullet=(K_\Gamma(R_r))_r$, and the supported
row is compared in its module category, hence after recovery in
$\operatorname{Mod}_{K_\Gamma(\mathbf Z_\ell)}(D(\mathbf Z_\ell))$.
The relative $\Gamma$-action belongs to the coefficient source.
External tame transport belongs to the actual geometric log source
and is retained by its specialization and star comparisons.
An additional finite semilinear descent diagram is separate data,
handled by Proposition~\ref{prop:frag-S-trace-naturality}.
The finite-carrier trace row is stated after forgetting external
base transport; a full infinite tame-equivariant trace row is not
asserted by its point-valued proof.  Forgetting that action takes
no invariants under base tame inertia.
...
The targets in this section are point-valued complexes and their
star-algebra modules, with external tame transport forgotten.
...
At a geometric point use $\omega_r=R_r$.  For the general-base
adic duality assertions fix a coherent derived-cartesian system
$\omega_{\mathscr S,\bullet}$ of dualizing complexes:
\[
 \omega_{\mathscr S,r+1}\otimes^L_{R_{r+1}}R_r
       \xrightarrow{\sim}\omega_{\mathscr S,r}.
\]
Require uniform bounded constructibility and, when a selected perfect
output is asserted, a common finite underlying-$R_r$ Tor interval
$[c_0,c_1]$ on a fixed finite stratification.  These are system
hypotheses, not consequences of levelwise dualizingness.
Verdier biduality is used only on the reflexive domain of these
complexes.  For
...
The notation $\mathbb D_{X,\mathrm{cont}}$ denotes the system of
finite Verdier duals, with a derived-limit realization only after
its coefficient reductions have been verified.  At a point this
verification for the continuous outputs follows from perfectness.
Over a general base, Lemma~\ref{lem:frag-iwasawa-adic-base-change}
proves it for the tensor-dualizable inputs specified there; it is
not asserted for every stratified-perfect input.
Whenever the finite six-functor comparisons form cartesian systems
on the stated domain, their actual mates assemble to
\begin{equation}
 \mathbb D_{\mathscr S,\mathrm{cont}}Rf_!
 \simeq Rf_*\mathbb D_{X,\mathrm{cont}}.
 \label{eq:frag-relative-verdier-mate}
\end{equation}
This notation asserts neither a general-base supported realization
nor a new exceptional functor on the bare infinite-root fibre.
...
Hom.  Over a base, for a uniformly lisse-perfect system, it denotes
the cartesian system of finite $R_r$-coefficient duals proved below,
with its ambient limit used only for recovery;
...
A coefficient is \emph{lisse-perfect} here if it is tensor-dualizable
on the whole base topos: locally a bounded complex of finite free
constant-coefficient sheaves.  This differs from being perfect only
on each constructible stratum.  For a uniformly lisse-perfect adic
base coefficient, with Hom interpreted in the adic system category
and computed levelwise, put
\begin{equation}
 C^{\vee_0}=R\mathcal Hom_{\mathbf Z_\ell}(C,\mathbf Z_\ell).
 \label{eq:frag-base-coefficient-dual}
\end{equation}
For $\mathcal L$ in the Verdier-reflexive domain, at each precision
tensor--Hom adjunction and the tensor dual of $C$ give the following
identity.  Its adic form uses compatible systems for $\mathcal L$
and its Verdier dual; reflexivity alone does not prove coefficient
change for an arbitrary constructible $\mathcal L$:
\begin{equation}
...
\begin{lemma}[Iwasawa base change and the coefficient-duality domain]
\label{lem:frag-iwasawa-adic-base-change}
Put $\Lambda=\Lambda_M^{\mathrm{Iw}}$, $\Lambda_r=\Lambda/\ell^r$,
and $\mathbf K_r=\mathbf K_M^{\mathrm{Iw}}\otimes^L R_r$.
Let $V_\bullet$ be any selected system of
Definition~\ref{def:frag-adic-iwasawa-sheaves} over a noetherian split
base, with common underlying-$R_r$ Tor interval $[a,b]$.  At a point
this is the entire source of
Proposition~\ref{prop:frag-point-source-iwasawa-admissibility}.
Write
\[
 H_{*,r}(V)=R\mathcal Hom_{\Lambda_r}(\mathbf K_r,V_r).
\]
The following domains are distinct.
\begin{enumerate}[label=\textup{(\roman*)}]
\item For every such selected system, $\mathbf K_r$ is a length-$d$
finite free resolution of $R_r$.  The actual coefficient maps give
\[
 H_{*,r+1}(V)\otimes^L_{R_{r+1}}R_r\simeq H_{*,r}(V).
\]
The outputs are constructible and stratified-perfect with common
Tor interval $[a,b+d]$.  Finite-projective Hom also commutes with
reduction of the ambient derived-limit realization.  Thus the full
general-base star and coefficient-comparison domain is unchanged.
\item Suppose additionally that each $V_r$ is lisse-perfect on the
whole base, uniformly in $r$.  Then the coefficient dual
$V_r^\vee=R\mathcal Hom_{R_r}(V_r,R_r)$, with contragredient inertia,
is a selected cartesian system with Tor interval $[-b,-a]$.
Its coefficient biduality map is an equivalence.  All these maps
respect the supplied finite semilinear descent and group inversion.
At a point no additional restriction is imposed; classical
Iwasawa-admissibility of the dual follows from the point-source theorem.
\item For inputs in (ii), use the coherent dualizing system of
Definition~\ref{def:frag-continuous-duality}, and put $N=d+e$ and
\[
 H_{!,r}(V)=\mathbb D_{\mathscr S,r}H_{*,r}(V^\vee)(-N)[-2N].
\]
These are cartesian, uniformly bounded constructible outputs.
With the stated Tor bound $[c_0,c_1]$ on the dualizing system they
are stratified-perfect with interval
$[a-d+c_0+2N,b+c_1+2N]$.
They recover by reduction from their ambient derived inverse limit.
At a geometric point $\omega_r=R_r$, so the bound is
$[a+2N-d,b+2N]$ and perfect integral recovery holds for the full
point source.  The finite cohomology towers there are Mittag--Leffler.
\end{enumerate}
Part (i) remains valid after tensoring with any selected stratified-perfect
base system.  Parts (ii)--(iii) remain valid after tensoring with a
uniformly lisse-perfect base system.  No common trivializing cover
or finite root level for all precisions is required.
\end{lemma}
\begin{proof}
The regular sequence $T_1,\ldots,T_d$ in $R_r[[T_1,\ldots,T_d]]$
gives the finite free augmentation resolution.  Applying Hom is a
finite complex of finite sums of $V_r$, with degrees from $0$ to $d$.
Its terms and differentials commute with scalar reduction.  This
proves (i), including its Tor bound, without exchanging a finite-group
bar with reduction.  Recovery of the original coefficient system in
the ambient category is Lemma~\ref{lem:frag-adic-completion}; the
finite free resolution then gives the asserted integral Hom formula.

For (ii), tensor duality on the whole base gives the actual maps
\[
 V_r\longrightarrow(V_r^\vee)^\vee,\qquad
 V_{r+1}^\vee\otimes^L_{R_{r+1}}R_r
       \longrightarrow(V_{r+1}\otimes^L_{R_{r+1}}R_r)^\vee.
\]
Both are equivalences by a local finite free computation; it also
reverses the Tor interval.  Dualizing the action uses group inversion,
so evaluation and coevaluation are equivariant and the maps retain
the given descent cells.  Their cohomology is again finite and
locally constant.  On each geometric stalk its continuous finite
inertia representation factors through a finite $\ell$-group;
over $R_r$ its augmentation ideal acts nilpotently.  Hence the dual
belongs to the selected action category and, by the already proved
relative action/Iwasawa equivalence, to the selected module model.
This argument uses tensor dualizability on the base, not a putative
commutation of internal Hom with restriction to an arbitrary stratum.

Put $H_r=H_{*,r}(V^\vee)$.  The same finite Koszul construction makes
$H_r$ lisse-perfect with Tor interval $[-b,d-a]$.  Consequently
\[
 H_{!,r}(V)=H_r^\vee\otimes^L_{R_r}
                  \omega_{\mathscr S,r}(-N)[-2N].
\]
The reduction map is the tensor product of the dual reduction map
for $H$ and the specified reduction map of $\omega_\bullet$.
It is therefore an equivalence with its inherited composition cells.
The displayed formula proves the stated bounds and constructibility;
with finite Tor for $\omega$ it proves stratified-perfectness.
Lemma~\ref{lem:frag-adic-completion} gives ambient recovery.
At a point use Lemma~\ref{lem:frag-point-perfect-recovery}; in each
finite target cohomology module, the descending images stabilize,
which proves the Mittag--Leffler assertion.  Tensoring preserves the
respective domains, giving the final clauses.
\end{proof}

\begin{remark}[Why the general-base duality domain is separate]
For a closed point $i:s\hookrightarrow S$ on a strictly henselian
nodal curve over an algebraically closed field, $i_*R_r$ is a selected
stratified-perfect cartesian system.  At $r=1$, set $F=\mathbf F_\ell$
and $D_0=R\mathcal Hom_S(-,F_S)$.  The normalization exact sequence
and purity on its two regular branches
\cite[Proposition 4.10.1]{LaszloOlssonFinite} give
\[
 i^!F_S\simeq F[-1]\oplus F(-1)^{\oplus2}[-2]=:C.
\]
Here the degree-one summand is the connecting term of the
normalization sequence.  Closed-immersion adjunction gives
$D_0(i_*F)=i_*C$ and $D_0^2(i_*F)=i_*R\operatorname{Hom}_F(C,C)$.
The latter has degree-zero dimension $1+2^2=5$.
Thus (ii) cannot hold on all stratified-perfect coefficients even
though (i) and the general-base category equivalences still do.
The distinction is between coefficient duality and Verdier duality;
replacing $V^\vee$ by a Verdier dual would change the stated formulas.
\end{remark}
...
relative-inertia-selected stratified-perfect constructible category and the removal of
...
All transition maps are $\Delta$-equivariant over the same $S$.
For the quotient assertion take the limit in stacks over the fixed
$\mathscr S=[S/\Delta]$.  A test map $T\to\mathscr S$ fixes one
$\Delta$-torsor $P\to T$ and its equivariant map $P\to S$.
A lift to the quotient at level $n$ is an equivariant lift of this
same $P$ to the split root fibre at level $n$.  A compatible family
of lifts, including their equivariance cells, is exactly a lift to
the split inverse limit: mapping into a limit preserves its limit
and the descent equations.  The same holds for morphisms of lifts.
Descent on the fixed torsor therefore proves the assertion over
$\mathscr S$.  It is not a general claim that taking orbit quotients
commutes with inverse limits merely because $\Delta$ is finite.
\end{proof}
...
The finite action-groupoid descent below uses the same fixed
$\Delta$-torsor over each test of $\mathscr S$ as in
Lemma~\ref{lem:frag-relative-fibre-limit}; no new orbit-quotient/limit
interchange is used.
...
For a point coefficient define the following two complexes.  Over a
split base, the star expression uses the full selected adic source;
the coefficient-dual partner and its adic properties below use the
lisse-perfect subsource of Lemma~\ref{lem:frag-iwasawa-adic-base-change}(ii).
All general-base expressions are interpreted levelwise in that model:
\begin{align}
...
Over a base the displayed $!$-expression is a coefficient-linear
partner, with dualizing object the constant coefficient unit.
The Verdier partner in Lemma~\ref{lem:frag-iwasawa-adic-base-change}(iii)
is instead this expression tensored with $\omega_{\mathscr S,\bullet}$.
At a geometric point these coincide.  Neither is by itself a
construction of a general-base geometric supported functor.
...
\]
In this product identity $\widetilde V$ is tensor-dualizable,
$\mathcal L$ lies in the stated Verdier domain, and the smooth
product projection supplies the relative purity factor.
...
For every coefficient in the full selected source,
$\mathsf K^{\mathrm{ad}}_{M,*}(V_{\mathrm{cont}})$ is naturally an
$A^{*,\mathrm{ad}}_M$-module.  On the coefficient-duality domain
specified above, $\mathsf K^{\mathrm{ad}}_{M,!}(V_{\mathrm{cont}})$
has the dual action; all
...
etale site, not arithmetic $K$-analytic cohomology.  The actual log
source can carry external tame transport, which its geometric star
comparisons retain.  This supplies neither an equivariant structure
on every abstract root coefficient nor an infinite tame-equivariant
finite-carrier trace target.  Additional finite descent is treated
with its supplied action diagram.
...
Let $\mathcal L$ be a stratified-perfect constructible adic object
on $\mathscr S$, and
...
This does not make a stratified-perfect base object tensor-dualizable.
Coefficient-dual cancellation uses a globally lisse-perfect tensor
factor and the Verdier-reflexive domain of
Definition~\ref{def:frag-continuous-duality}; the general-base adic
duality conclusions use Lemma~\ref{lem:frag-iwasawa-adic-base-change}.
No such extra condition is added to the constructible star source.
